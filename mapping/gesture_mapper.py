...
from utils.models import MotionFeatures, ScaleNote, SoundParameters
...
class GestureMapper:
    ...
    def map(self, motion: MotionFeatures) -> SoundParameters:
        ...
        note_index = round(motion.x * (len(self._scale) - 1))
        note = self._scale[note_index]

        amplitude_span = self._config.max_amplitude - self._config.min_amplitude
        amplitude = self._config.min_amplitude + (1.0 - motion.y) * amplitude_span
        brightness = _clamp(
            (motion.velocity * self._config.velocity_weight)
            + (motion.openness * self._config.openness_weight)
        )
